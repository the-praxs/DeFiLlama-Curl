...
def get_current_prices(
    tokens: List[Dict[str, str]], search_width: str = "6h"
...
def get_historical_prices(
    ...
    *Endpoint: GET /prices/historical/{timestamp}/{coins}*
    ...
    tokens = arg_parser(tokens, format="normal")
    url = (
        f"{BASE_URL}/prices/historical/{timestamp}/{tokens}?searchWidth={search_width}"
    )
...
def get_historical_batch(
    tokens: Dict[str, List], search_width: str = "6h"
) -> Dict[str, Dict[str, any]]:
...
def get_nearest_block(chain: str, timestamp: int) -> Dict[str, int]:
